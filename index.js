...
【ご連絡事項（あれば）】`;
    ...
    if (text.includes('お名前') && text.includes('所属道場')) {
      const rawDate = getField(text, '【希望日時】');
      const normalizedDate = toHalfWidth(rawDate);
      const match = normalizedDate.match(/(\d{1,2})月(\d{1,2})日/);
      ...
      const confirmMessage = `${parsed.name}さん\nご回答ありがとうございます。\n\n${parsed.date}\n場所：${place}\n\n参加費：3,000円　家族割あり（2名以上で1名につき1,000円割引）\n\n前日に再度ご連絡いたします。\n不明点はLINEの「お問い合わせ」からどうぞ。`;
      await sendLineMessage(confirmMessage, userId);
    }
  }
...
app.get('/broadcast/visitors', async (req, res) => {
  ...
  res.send(`予約者${visitorSnapshot.size}名に送信しました`);
});
